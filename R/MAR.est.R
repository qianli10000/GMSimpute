...
MAR.est<-function(abundance,sample,log.scale=TRUE,violin.plot=FALSE){
  ...
  colnames(test_edata)=colnames(result_edata)[seq(1,8,2)]
  for (i in 1:(n_sample/2)){ 
  ...
  if(violin.plot){
    ...
    p=ggplot(plot.data,aes(x=sample, y=value,color=Compound))+geom_violin(position = dodge)+geom_boxplot(width=0.1,position = dodge)+theme(legend.position="bottom")
